...
def setup_bots():
    ...
    if config.Discord.lower() == "true":
        discord_bot_client = DiscordBotClient(config.DiscordClientBotToken, config.DiscordChannelID,
                                              config.DiscordDalleBotID)
...
if __name__ == "__main__":
    try:
        uvicorn.run(app="index:api.app", host="127.0.0.1", port=config.PORT)
