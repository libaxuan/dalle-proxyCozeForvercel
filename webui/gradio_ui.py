...
footer_html = '''
                <div id="footer">
                    <a href="/docs">API</a>
                     • 
                    <a href="https://github.com/libaxuan/dalle-proxyCozeForvercel">Github</a>
                </div>
              '''
...
with gr.Blocks(css='webui/custom.css',js='webui/localStorage.js') as demo:
    gr.Markdown("# 文本到图像")
    gr.Markdown("输入一些文本并提交以生成一张图片。")
    with gr.Row():
       with gr.Column():
           api_key_input = gr.Textbox(label="输入密钥", type="password",
                                      placeholder="如果您有API密钥，请在这里输入。",
                                      visible=False if (config.Web_share.lower()=="true" or not config.Key) else True
                                      )
           input_text = gr.Textbox(label="Prompts",lines=2, placeholder="在这里输入文本...")
